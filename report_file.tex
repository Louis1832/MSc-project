...
There are three distinct types of missing data, missing at random (MAR), missing not at random (MNAR) and; missing completely at random (MCAR). Data is considered MCAR when the missingness of the data is random. This differs from MNAR which describes missing data that depends on the context behind the missing value. For example people are less likely to disclose their substance use due to societal judgement and/or its legality. And finally MAR is dependent on data from other variables\cite{Yan2025}. The importance of these distinctions lie in the determination of the choice of imputation or deletion method of dealing with this missingness.
...
Regression imputation uses data from other variables and imputes with regression techniques, such as those used to predict the length of stay in this study. It is worth noting that both regression and hot deck require the variables used in the imputation to be fully-observed. Therefore, these methods are less effective for imputing datasets that contain a lot of missing data. 

Single imputation is not the optimal method when missingness in the dataset is more prevalent and therefore advanced methods should be considered.
