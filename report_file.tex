...
Cancer is a disease that involves the uncontrollable multiplication of malignant cells, in some cases forming tumours in organs or tissues. In the US, and many other countries around the world, cancer has become the leading cause of death, and in the US specifically it has been reported that around 39\% people will be diagnosed with any cancer in their lifetimes\cite{cancerCancerSite}. In 2025 it was reported that around 12\% of cancer cases were lung cancer cases\cite{cancer}. These statistics indicates that the volume of patients requiring ICU treatment may be significantly large and therefore the optimisation of length of stay analysis is key for adequate resource management and patient care planning. Lung cancer in particular describes the cancerous cell growth in the lungs and is regarded as a serious health issue for both the patient and healthcare systems worldwide.

%According to the official WHO global cancer observatory, lung cancer is the cancer with the highest percentage of mortality and one of the most prevelant. To link this to the statistic previously mentioned, lung cancer will develop in approximately 1 in 17 and be the cause of death in 1 in 28 of Americans.
This prevelance differs when broken down by sex and lung cancer is not the most prevelant cancer for women, breast cancer is, or for men where prostate cancer is the most prevelant however when addressing the whole population lung cancer is the cancer that poses the biggest threat. Adressing both males and females, the mortality rates are higher than any other cancer. Focusing on the one cancer that is singled out as the most threatening allows for a more concentrated study that should in theory generate more useful results. 

As of Joohyn Park., et al's 2019 report\cite{Park2019}, the mean annual health care expenditure for a lung cancer patient in the US is \$35,141. This was significantly higher than the 4 most common cancer types that were included and approximately 8-9 times as high for a cancer free patient, \$4,484. These numbers point towards the extent of how significant the burden of lung cancer is on the US healthcare system. They also report that lung cancer tends to be diagnosed at more advanced stages so the survival duration is short. A separate, more recent, report notes the estimated 5 year survival rate at 27\% as of 2025 \cite{cancer}. With a higher number of cases diagnosed at advanced stages, intensive care units are expected to be required to administer treatment to patients.
...
\item The degree to which the sample data is inclusive of various subgroups of the wider population. For example, if the majority of the sample group are ethnically white the model may have less reliable results for individuals from other ethnic groups.
...
Length of stay predictions can contribute to a more educated forecast for the ward's running cost. With this information the hospital can generate a more realistic budget which may reduce the likelihood of excessive spending and underfunding.

At current clinicians do not use machine learning techniques to predict length of stay for lung cancer ICU patients. Their predictions or estimates almost primarily come down to clinical judgement. Therefore estimates may vary depending on a clinicians experience and familiarity to the patient. This prediction is also not an immediate priority for the clinician, they may have the most granular knowledge but they are more focused on administering the best possible treatment to their patients. Therefore a clinically significant machine learning prediction model could help as an aide for hospital staff to make good approximations of the length of stay for an individual patient\cite{DosSantos2025}.

%The knowledge of how long a patient will stay on the ward is a useful bit of information for managing costs. Since longer stays mean higher costs, length of stay predictions can be used to estimate how much a hospital needs to be spending on the ICU ward which distinguishes how much of a hospital's overal budget goes toward the ICU.

\subsection{The MIMIC Database}
MIMIC is a large relational database that consists of deidentified data collated from thousands of admissions to the intensive care unit of the Beth Israel Deaconess Medical Centre in Boston, Massachusetts\cite{physionetMIMICIVV31}. The acronym MIMIC has stood for ``Multiparameter Intelligent Monitoring in Intensive Care" and more recently ``Medical Information Mart for Intensive Care". Some key features of this database are that it is the only freely accessible critical care database of its kind and contains detailed information on patients spanning more than a decade. 

The most recent version of the MIMIC database is MIMIC-IV. It includes data collected from January 2008 to December 2022, which is an extension on MIMIC-III which recorded data up to December 2012. Using the updated version should allow for greater relevance and accuracy of the prediction model.

The database is split into four modules, the core data, the hospital data, the ICU data and the emergency department data. For my study I have not included the data from the emergency department module. Within the core there are three tables to store the non-clinical data that can be used for linkage purposes; these are labeled admissions, patients and transfers. The hospital module is the largest, containing nineteen tables. There are tables for administration (services, poe, poe\_detail), billing (diagnoses\_icd, d\_icd\_diagnoses, procedures\_icd, d\_icd\_procedures, drgcodes, hcpcevents, d\_hcpcs), measurement (microbiologyevents, labevents, d\-labitems, omr) and medication (emar, emar\_detail, pharmacy, prescriptions). The ICU module includes the d\_items, chartevents, datetimeevents, ingredientevents, inputevents, outputevents and procedureevents tables for recording measurement data and the icustays table for linkage.

%The value of MIMIC lies in the comprehensive real-world data it contains.
% which enables the model to train more accurately compared to synthetic data, allowing it to fit better to unseen data.

Data can be accessed after completing a short ethics course set up by the CITI programme website\cite{physionetCITICourse}\cite{citiprogramTechEthics}.
...
The main aim of this project is to build accurate and efficient machine learning models using data from the MIMIC-IV dataset to enhance the prediction of the length of stay in the Intensive Care Unit, ICU, for lung cancer patients. The ideal result would be for these models to assist in the management of lung cancer patients in intensive care wards in terms of cost and effectiveness.

\subsubsection{Objectives}

The main objectives are the following:
\begin{itemize}
    \item Extract the data from the MIMIC dataset to form a final dataset ready to be used to develop predictive models.
	%\begin{itemize}
	%\item Objective 1 - Determine the variables for selection 
	%\item Objective 2 - Setup the MIMIC database on PostgresSQL
	%\item Objective 3 - Extract the data
	%\item Objective 4 - Clean the data
	%\end{itemize}
    \item Train various predictive models via nested cross validation. The missing data imputation will occur within the cross validation process.
	%\begin{itemize}
	%\item Objective 1 - Cross-validate 
	%\item Objective 2 - Impute the missing data
	%\item Objective 3 - Linear regression model
	...
	%\item Objective 5 - Random forest
	%\end{itemize}
    \item Evaluate the values of each model's performance metrics and variable importance and compare them against each other.
	%\begin{itemize}
	%\item Objective 1 - Calculate performance metrics
	%\item Objective 2 - Compare models
	%\item Objective 3 - Compare findings with other studies
...
To create the schema, i.e. the layout and structure of the database being created, a SQL script is run, specifically tailored for MIMIC-IV, to construct empty tables ready for the data export process. The script was retrieved from a Github repository.\cite{SQLcode}
...
The patient must be an adult, as in over 18, at the time of their admission into hospital.

The patient must have at least one ICU admission within their hospital stay.

The patient must have a relevant ICD-9 or ICD-10 code that indicates a diagnosis of lung cancer.

The patients' stay must be the first in their admission.
...
\begin{longtable}{ | L{2cm} | L{3cm} | L{8cm} | } 
    \hline
    \textbf{ICD Coding System} & \textbf{ICD Code} & \textbf{Condition Description}  \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
    \hline
    \endfirsthead
...
\hline
\endhead
ICD-9 & 162.X & Malignant neoplasms of the trachea, bronchus and lung. I.e., cancer that originates in the lung  \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.05mm}}
    \hline
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
ICD-10 & C34.X & Malignant neoplasms of the bronchus and lung  \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.05mm}}
    \hline
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
ICD-10 & C78.0 & Secondary malignant neoplasm of lung. I.e., lung cancer that develops as a result of a previous cancer  \\ 
    \hline
\caption{Caption}\label{tab:my_label}\\
\end{longtable}
\end{center}}}
...
Should there be multiple values before the first ICU admission the value recorded closest to the admission is extracted. 
%Only data recorded within at least 1 hour of being admitted to the ICU ward were extracted.
...
To predict length of stay the model will need relevant data as its input. Data from variables of interest are collected, these are refered to as the covariates. The table below gives the list of covariates to extract from MIMIC, selected for their use in relevant literature identified in the literature review section and for their relevance. 
...
For each continuous covariate the data value that is used in the prediction is the value that is taken closest to the admission time. 

\subsection{Data Cleaning}
SQL and R programming [Ref] were used to handle data cleaning.
...
Some of the categorical variables, extracted from the dataset, had many specific levels with some including an insufficiently low amount of individuals. For example the ethnicity variable was split down into many groups, some caputuring only a few individuals. Each group may be incorporated into a more broad ethnic umbrellas. For example, white - eastern/european and white - russian are recognised as different ethnic groups in MIMIC, but both can be included within a white ethnicity category. Individuals from ethnic groups that don't fit into a specific category are placed in a category labelled ``Other". Admission type categories were also aggregated. Any missing data in a categorical variable was placed into its own missing data group.

%The limitations of this are apparent in that the individuals in this group have a significant amount of variance in their ethnic characteristics which is not helpful when using ethnicity as a predictor in a prediction model. 

%Also the aggregation of groups in this way reduces the validity of the true impact of ethnicity on a patient's length of stay. It is important to aggregate the categories in this way for personal security purposes, if a patient is known to be the only patient in a certain category with a lung cancer intensive care stay they would be easily identifiable. 
...
There are three distinct types of missing data, missing at random (MAR), missing not at random (MNAR) and; missing completely at random (MCAR). Data is considered MCAR when the missingness of the data is random\cite{Mack2018}. This differs from MNAR which describes missing data that depends on the context behind the missing value. For example people are less likely to disclose their substance use due to societal judgement and/or its legality. And finally MAR is dependent on data from other variables\cite{Yan2025}. The importance of these distinctions lie in the determination of the choice of imputation or deletion method of dealing with this missingness.

Dealing with missing data can involve the complete case analysis omission of records or imputation. Although omission is the simpler method, the resulting model is subject to bias as there due to the possibility for certain groups to have missing data, of the type MAR or MNAR, in some variables\cite{Kang2013}. Therefore, omission of these data could potentially lead to a misrepresentative study population which in turn leads to the aforementioned bias in the prediction models\cite{Peng2023}.

There are some forms of imputation that are a much more valid approach for minimising bias. Imputation is, in simplistic terms, the filling in of missing data, as an approximation, to provide a full dataset for analysis. The imputed data is based on the statistical characteristics of the observed data such as its distribution. 

%For imputing missing data on a normal distribution if the a variable's distribution is normal, the imputed data should lie somewhere on the same normal distribution with the imputation method mapping the data in this way. 
...
Single imputation is a basic imputation method that imputes the missing data with single estimated values. It is most important to note that these methods only impute with one iteration. One way to singularly impute the missing data is to replace it with a summary statistic, such as the mean or median for quantitative variables or mode for qualitative variables\cite{Dettori2018}. For datasets with a significant amount of missing data this imputation method is less suitable and not recommended for producing robust estimates as the variance of the resulting imputed distribution would almost certainly be a poor estimation of the variance of the true distribution. 

Other methods offer an improvement to this simplistic form of imputation. The hot deck method matches individuals based on the similarity of their characteristics\cite{Andridge2010}, and uses this information to impute the missing data in the dataset. The individual with the missing data is labelled the recipient and the individual with the data used to impute the missing value is labelled the donor. How the donor is selected is dictated by the type of hot deck imputation \cite{Georgiadis2025}. As opposed to imputing all missing data with the same value, methods such as hot deck or regression give imputations that differ for each missing value depending on the characteristics of their records. 

Regression imputation uses data from other variables and imputes with regression techniques. It is worth noting that for the regression and hot deck methods the accuracy of the imputations relies on the availability of observed data in the dataset. Single imputation is not the optimal method when missingness in the dataset is more prevalent and therefore advanced methods should be considered.\cite{Woods2021}
%, such as those used to predict the length of stay in this study. 
...
The number of terms in multivariate regression is represented as the letter \(p\) in the equation. 

%A more succinct way to express this equation is to write it in matrix form \cite{ThePennsylvaniaStateUniversity2024} as follows.
...
To explain the random forest method, the concept of a classification and regression decision tree (CART) must be established first\cite{Ibrahim2023}. At the beginning of constructing a decision tree \(100\%\) of the data constitutes what is labelled the root node\cite{SinghKushwah2022}. The root is split into two groups/child nodes, according to an attribute of one independent variable\cite{Ishwaran2015}, so that the overall mean squared value is optimally minimised\cite{Kitchin2020} . The splitting continues until a stopping point is reached. This may be because the mean squared error minimisation offers insignificant improvements or when the process has concluded in a predetermined maximum number of splits. As a result the data in the terminal nodes should have a significant level of homogeniety. Each terminal node includes a subset of the overall population wherein the mean for the values of the target variable constitutes their predicted value \cite{SpringerSeries}. Therefore, when evaluating the model, the test data is fed into the tree and their predictions are determined according to which terminal node they fall into. The evaluation metric can then be calculated using these predictions in comparison to the actual values.

\includegraphics[width = 150mm, scale = 0.75]{decisiontreegraph}

%  is split in two and then two again and so on. How these splits are determined is based on attributes of the independent variables used in the model. When training a decision tree the independent variables are used to determine how to split the data into the two new nodes, or leaves. Should a leaf be split in a way that minimises the mean squared error by a significant amount it will carry on splitting. A leaf with no further splits is considered to be a terminal node. Each terminal node includes a subset of the overall population wherein the mean for the values of the target variable in this subset constitutes the predicted value \cite{SpringerSeries}. Therefore when evaluating the model the test data would be fed into the tree and the predictions obtained depend on which leaf each individual falls into. The evaluation metric can then be calculated using these predictions and the actual values.

%How the algorithm splits the data is determined by how much a variable or multiple variables minimise the mean squared error after the split\cite{Kitchin2020} and the same is true for the decision on which leaf to split further.

A single decision tree achieves low training error due to its high flexibility, meaning that the model is more likely to fit to noise in the data. Thereby the model is prone to overfitting. The lack of generalisability on unseen data, especially when making predictions on the test set, is a major drawback for using a single decision tree as a prediction model. To mitigate overfitting and reduce variance, multiple decision trees can be aggregated within an ensemble framework. One such ensemble method, that is widely used, is the random forest algorithm\cite{Biau2015}. The final predictions that are obtained from a random forest model are averages from the predictions made by multiple independent, parallel decision trees\cite{Breiman2001}. The data used in each decision tree is sampled with replacement, a method known as bagging (bootstrap aggregation). Since each tree sees different data, the overall predictions are less affected by noise and variable errors which cancel out when averaging the predictions resulting in lower variance. Another feature of the algorithm is random variable selection. At each split only a subset of variables are considered. This promotes diversity among the trees. A random forest model is therefore more robust. 

% is subject to overfitting, as the tree fits very closely to the training data. Its noise influences the p thereby its predictions are influenced by the training sets noise and therefore lacking generalisation particularly when it is used to make predictions on the test data In CART trees, classification and regression trees, only one variable is selected\cite{Ishwaran2015} and these are the type of trees that are used as the makeup for the ensemble random forest model.  
...
The RMSE is a metric that measures the standard deviation of the residuals in the model. An advantage of using this metric is that it is in the same units as the target variable which adds ease to interpretability. In relevance to this model the RMSE would represent how far off, typically, in days a prediction may be from the actual value. 

MAE, in a sense, shows the same measure as RMSE in that it measures how much a model's predictions deviate from the actual values. The difference between the two is how this figure is calculated. MAE takes the absolute value whereas RMSE takes the squared. Since RMSE penalises larger values more than smaller ones it is more sensitve to outliers than MAE. This may be useful if outliers are more costly in context.

The metric R squared measures the proportion of the dependent variable's variance that can be predicted from the independent variables.
...
Other validation methods may be used but they come with significant downsides. For example the jackknife and bootstrap methods are not suitable because they are much less effective for larger datasets due to their high computational complexity and therefore computational cost. Bootstrap can introduce bias and it is not reasonable to use for a length of stay prediction as the length of stay is often highly skewed to the right which bootsrapping doesn't handle well. Jackknife mutes the potential for exploring nonlinear variables as for this it is unstable. 
...
%This figure details how the outer fold is realised. Firstly the incomplete data set is split into 5 folds. In each fold a seperate section is used as a test set and train set. The validation within the fold is carried out just as in the previous diagram. the 5 resulting validation metrics are averaged to give the final validation metric for the model.

%The incomplete dataset is split into five folds with each fold's data split in two with four fiths of the data consisting the training set and the rest used as the test set. Within each fold the model evaluation is carried out as detailed in the previous figure. The five validation metrics obtained from this are averaged to obtain the final validation metric.

The incomplete dataset is partitioned into five folds. In each iteration, one fold is held out as the test set, while the remaining four folds are used as the training set. Within each outer fold, the full modelling pipeline (as described in the previous figure) is carried out, including any preprocessing, imputation, and model fitting procedures. This produces one validation metric per fold. The five fold-level validation metrics are then averaged to obtain the final estimate of model performance.
...
Analysing the representation of variables in linear regression models allows for a rough comparison of their importance.

An essential data pre-processing step to ensure valid interpretations is to scale continuous data so that for each variable the mean is 0 and the standard deviation is 1. Doing so places each variable on the same scale where the coefficients correspond to the effect of a 1 standard deviation change. Dummy coded categorical variables cannot be placed on this scale, however. Their coefficients represent the effect of being in the category as opposed to being in the reference category.

A forest plot can be used to visualise the comparison.

\subsubsection{Random Forest}

The out of the bag data used to evaluate the model can also be used to find the importance of each variable.

To start with, in a single tree, the original MSE is calculated. To find the importance of a single variable its predictive effect is removed and a new MSE is calcuated. To remove the predictive effect of the variable its data are shuffled so that the values belong to the wrong records. The increase in MSE as a percentage is taken as a result. This is repeated for each variable in the data then the whole process is repeated in every other tree in the model. The overall measure of importance is the average percent increase in MSE where higher percent increase indicates higher importance.

%The random forest model measures variable importance by the average \(\%\)  of MSE increase.  after the removal of the predictive effect
...
The linear regression model is the most successful according to the MAE metric with just over a 0.1 hour difference between the linear regression's MAE of 2.79 to the other three that are all either 2.9 or 2.91. Where this is the case for the MAE the inverse is true for both the RMSE and the R\(^2\) values. In this case linear regression was worst performing model by a significant amount. Random forest beat out the two penalisation methods and would therefore be considered the superior model based on these metrics alone.
...
\hline
\endhead
\textbf{Linear Regression} & 5.69 & 2.79 & -0.24  \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.05mm}}
    \hline
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
\textbf{LASSO} & 5.05 & 2.90 & 0.03  \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.05mm}}
    \hline
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
\textbf{Ridge} & 5.05 & 2.90 & 0.03 \\ 
    \noalign{\global\setlength{\arrayrulewidth}{0.05mm}}
    \hline
    \noalign{\global\setlength{\arrayrulewidth}{0.7mm}}
\textbf{Random Forest} & 4.99 & 2.91 & 0.04 \\ 
     \hline
\caption{Caption}\label{tab:my_label}\\
\end{longtable}
\end{center}}}
...
\subsection{Findings}
Out of the four prediction models random forest was found to be the most reliable  

\subsection{Comparison to the Literature}
Less accurate findings than the literature found although there is a higher possibility that more significant findings will be published, other paper used more data variables and had less missing data, less prior literature used the MIMIC-IV database, similar result was found in terms of which machine learning method was most effective/suitable

\subsection{The Effect of Missing Data}
missing data levels were high resulting in an inaccurate and less clinically and statistically significant result, imputation methods only work well when missing data proportion is minimal, complete case analysis would not be feasible as the sample size would be below the required amount

\subsection{Limitations}
A lot of missing data, not taking into account of those who died during their stay, only a small amount of variables, limited prior study on the specific topic

\subsection{Future Recommendations}
larger datasets, assess on data with less missing data or a significant sample size of complete case data, investigate new variables, take into account the patients who die during stay, explore new algorithms, test on new data/demographics

\section{Conclusion}
Findings were not clinincally significant enough for this model to be trusted to make accurate predictions on lung cancer patients length of stay in the ICU but due to the depths of its limitations the possibility for an effective prediction model such as this to be created cannot be ruled out. Further study with fewer limitations may provide a more reliable prediction model
