WITH haematocrit_records AS (
SELECT ce.subject_id, ce.charttime, ce.hadm_id, icu.stay_id, ce.valuenum AS haematocrit, icu.intime, icu.outtime, icu.los,
...
WHERE LOWER(di.label) LIKE '%hematocrit%'
  AND ce.charttime < icu.intime - INTERVAL '1 hour'
  AND ce.valuenum IS NOT NULL)
  
SELECT *
FROM haematocrit_records
WHERE rn = 1
