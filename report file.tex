...
To explain the random forest method, the concept of a decision tree must be established first. At the beginning of constructing a decision tree \(100\%\) of the dependent variable data constitutes what is labelled the root node of the tree [cite decision trees]. From there this node is split in two. How this is determined is based on attributes of the indepent variables used in the model. When training a decision tree the independent variables are used to determine how to split the data into the two new nodes. From here the node that includes the most data is split further and this continues until splitting the data once more would not improve the fit of the model significantly. A node that will not be split further is labelled a leaf and at every node the prediction of the dependent variable is the mean of the values that are included within it. Therefore when evaluating the model the test data would be fed into the tree and the value of the leaf that it ends up in is the prediction for that individual. The evaluation metric can be calculated using these predictions and the actual values.
...
It is worth noting that this data has been randomly generated and in no way models the actual relationship between the price of a product and its subsequent sale figures.
...
Random forest is an ensemble method, one that is an ensemble of multiple decision trees. This model applies a bootstrapping technique to sample with replacement data from the train dataset to create D new datasets that will be used to train D trees. The number of trees is to be determined and tuned as a parameter of the model. With random forest the number of variables to consider for each split is limited to the hyperparameter mtry where mtry is less than the number of predictor variables. A larger mtry value will increase the amount of bias whereas the a low mtry value will increase the amount of variance, so finding the optimal value is key to find a middle ground in reducing the variance and the bias. This hyperparameter is very influential as it is what separates the random forest out from other models. The D trees therefore constitue the forest part of the algorithm each giving separate predictions for the data. The next step of the algorthim is to aggregate the predictions from each of the trees. Since this is done with a k-cross validation these steps are repeated k times. Then the model can be evaluated according to the process of using the test dataset to test the model and produce the values for the performance metrics. 
...
\subsection{cross validation to imputation pipeline}
