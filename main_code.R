...
main %>%
  select(c("temperature", "platelets", "glucose", "chloride", "potassium", "ptt",
           "haemoglobin", "troponin", "inr", "sodium", "race", "insurance")) %>%
  ...
  mutate(Insurance = na_if(Insurance, "Unknown")) %>%
  ...
  scale_fill_manual(values = c("Temperature"= "firebrick2",
                               ...
                               "Ethnicity" = "darkolivegreen3",
                               "Insurance" = "darkolivegreen3")) +
...
main_impute_2 %>% 
  select("potassium", "sodium", "glucose", "haemoglobin",
         "chloride", "platelets", "insurance", "ethnicity", "haematocrit") %>%
...
#predictor_matrix_3["ethnicity", "total_los"] <- 0
#predictor_matrix_3["insurance", "total_los"] <- 0
#predictor_matrix_3["platelets", "total_los"] <- 0
#predictor_matrix_3["sodium", "total_los"] <- 0
#predictor_matrix_3["chloride", "total_los"] <- 0
#predictor_matrix_3["potassium", "total_los"] <- 0
#predictor_matrix_3["glucose", "total_los"] <- 0
#predictor_matrix_3["haemoglobin", "total_los"] <- 0

imp_main_4 <- mice(main_impute_3, m = 5, maxit = 5, predictorMatrix = predictor_matrix_3)

imp_main_4$method

imp_main_4_data <- complete(imp_main_4, "all")

meth_main_4 <- imp_main_4$method

predictor_matrix_main_4 <- imp_main_4$predictorMatrix

##############################LOGISTIC REGRESSION VARIABLE ASSOCIATION########################
imp_main_4_data_log <- complete(imp_main_4, 1) %>%
  left_join(main_impute_3, by = c("age", "sex", "admission_type", "ondansetron",
                                  "calcium_gluconate", "total_los"),
            suffix = c("_imp", "_orig")) %>%
  mutate(missing_eth = as.integer(is.na(ethnicity_orig))) %>%
  mutate(missing_ins = as.integer(is.na(insurance_orig))) %>%
  mutate(missing_plate = as.integer(is.na(platelets_orig))) %>%
  mutate(missing_gluc = as.integer(is.na(glucose_orig))) %>%
  mutate(missing_chlo = as.integer(is.na(chloride_orig))) %>%
  mutate(missing_pot = as.integer(is.na(potassium_orig))) %>%
  mutate(missing_haem = as.integer(is.na(haemoglobin_orig))) %>%
  mutate(missing_sodi = as.integer(is.na(sodium_orig))) %>%
  select(-ethnicity_orig) %>%
  select(-insurance_orig) %>%
  select(-platelets_orig) %>%
  select(-glucose_orig) %>%
  select(-chloride_orig) %>%
  select(-potassium_orig) %>%
  select(-haemoglobin_orig) %>%
  select(-sodium_orig) %>%
  rename(ethnicity = ethnicity_imp) %>%
  rename(insurance = insurance_imp) %>%
  rename(platelets = platelets_imp) %>%
  rename(glucose = glucose_imp) %>%
  rename(chloride = chloride_imp) %>%
  rename(potassium = potassium_imp) %>%
  rename(haemoglobin = haemoglobin_imp) %>%
  rename(sodium = sodium_imp)

imp_data_log = imp_main_4_data_log %>% 
  select(c("age", "sex", "admission_type", "ethnicity", "insurance", "ondansetron",
           "calcium_gluconate", "total_los", "glucose", "sodium", "chloride",
           "platelets", "potassium", "haemoglobin"))

imp_data_log_2 = imp_main_4_data_log %>% 
  select(c("age", "total_los", "glucose", "sodium", "chloride",
           "platelets", "potassium", "haemoglobin"))

corr_matrix <- cor(imp_data_log_2, method = "spearman")

corrplot(corr_matrix)

corrplot(imp_data_log_2, tl.col = "black", method = "spearman")

ggpairs(imp_data_log_2)

log_eth <- glm(missing_eth ~ age + sex + admission_type + insurance +
                 platelets + glucose + chloride + potassium + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_ins <- glm(missing_ins ~ age + sex + admission_type + ethnicity +
                 platelets + glucose + chloride + potassium + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_plate <- glm(missing_plate ~ age + sex + admission_type + insurance +
                 ethnicity + glucose + chloride + potassium + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_gluc <- glm(missing_gluc ~ age + sex + admission_type + insurance +
                 platelets + ethnicity + chloride + potassium + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_chlo <- glm(missing_chlo ~ age + sex + admission_type + insurance +
                 platelets + glucose + ethnicity + potassium + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_pot <- glm(missing_pot ~ age + sex + admission_type + insurance +
                 platelets + glucose + chloride + ethnicity + haemoglobin +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_haem <- glm(missing_haem ~ age + sex + admission_type + insurance +
                 platelets + glucose + chloride + potassium + ethnicity +
                 sodium + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

log_sodi <- glm(missing_sodi ~ age + sex + admission_type + insurance +
                 platelets + glucose + chloride + potassium + haemoglobin +
                 ethnicity + ondansetron + calcium_gluconate + total_los,
               data = imp_main_4_data_log,
               family = binomial)

##############################FOR LOOP SETUP####################################

#predictor_matrix_main_4["insurance","total_los"] <- 0
#predictor_matrix_main_4["insurance","sex"] <- 0
#predictor_matrix_main_4["insurance","ethnicity"] <- 1
#predictor_matrix_main_4["insurance","platelets"] <- 0
#predictor_matrix_main_4["insurance","ondansetron"] <- 0
#predictor_matrix_main_4["insurance","potassium"] <- 0
#predictor_matrix_main_4["insurance","calcium_gluconate"] <- 0
#predictor_matrix_main_4["ethnicity","sex"] <- 1
#predictor_matrix_main_4["ethnicity","potassium"] <- 0
#predictor_matrix_main_4["ethnicity","haemoglobin"] <- 0
#predictor_matrix_main_4["ethnicity","ondansetron"] <- 1
#predictor_matrix_main_4["platelets","sex"] <- 1
#predictor_matrix_main_4["platelets","ethnicity"] <- 0
#predictor_matrix_main_4["platelets","potassium"] <- 0
#predictor_matrix_main_4["platelets","haemoglobin"] <- 0
#predictor_matrix_main_4["platelets","sodium"] <- 1
#predictor_matrix_main_4["platelets","ondansetron"] <- 1
#predictor_matrix_main_4["platelets","total_los"] <- 0
#predictor_matrix_main_4["glucose","age"] <- 1
#predictor_matrix_main_4["glucose","sex"] <- 0
#predictor_matrix_main_4["glucose","ethnicity"] <- 1
#predictor_matrix_main_4["glucose","platelets"] <- 1
#predictor_matrix_main_4["glucose","chloride"] <- 1
#predictor_matrix_main_4["glucose","potassium"] <- 1
#predictor_matrix_main_4["glucose","haemoglobin"] <- 1
#predictor_matrix_main_4["glucose","sodium"] <- 1
#predictor_matrix_main_4["glucose","ondansetron"] <- 0
#predictor_matrix_main_4["glucose","total_los"] <- 0
#predictor_matrix_main_4["potassium","age"] <- 0
#predictor_matrix_main_4["potassium","sex"] <- 0
#predictor_matrix_main_4["potassium","ethnicity"] <- 1
#predictor_matrix_main_4["potassium","platelets"] <- 0
#predictor_matrix_main_4["potassium","haemoglobin"] <- 0
#predictor_matrix_main_4["potassium","ondansetron"] <- 1
#predictor_matrix_main_4["potassium","total_los"] <- 0
#predictor_matrix_main_4["potassium","calcium_gluconate"] <- 0
#predictor_matrix_main_4["chloride","age"] <- 1
#predictor_matrix_main_4["chloride","sex"] <- 0
#predictor_matrix_main_4["chloride","ethnicity"] <- 1
#predictor_matrix_main_4["chloride","haemoglobin"] <- 1
#predictor_matrix_main_4["chloride","ondansetron"] <- 1
#predictor_matrix_main_4["chloride","total_los"] <- 0
#predictor_matrix_main_4["sodium","age"] <- 1
#predictor_matrix_main_4["sodium","sex"] <- 0
#predictor_matrix_main_4["sodium","ethnicity"] <- 1
#predictor_matrix_main_4["sodium","platelets"] <- 0
#predictor_matrix_main_4["sodium","haemoglobin"] <- 1
#predictor_matrix_main_4["sodium","ondansetron"] <- 0
#predictor_matrix_main_4["sodium","total_los"] <- 0
#predictor_matrix_main_4["sodium","calcium_gluconate"] <- 0
#predictor_matrix_main_4["haemoglobin","age"] <- 0
#predictor_matrix_main_4["haemoglobin","sex"] <- 1
#predictor_matrix_main_4["haemoglobin","ethnicity"] <- 0
#predictor_matrix_main_4["haemoglobin","platelets"] <- 0
#predictor_matrix_main_4["haemoglobin","chloride"] <- 1
#predictor_matrix_main_4["haemoglobin","potassium"] <- 0
#predictor_matrix_main_4["haemoglobin","sodium"] <- 1
#predictor_matrix_main_4["haemoglobin","ondansetron"] <- 1
#predictor_matrix_main_4["haemoglobin","total_los"] <- 0
#predictor_matrix_main_4["ethnicity","total_los"] <- 0
...
results_3 <- foreach(x = seq_along(folds),
                     .export = c("full_model_3", "meth_main_4",
                                 "predictor_matrix_main_4", "m",
                                 "main_impute_3")) %dopar% {
                                   #Define train and test data
                                   test_id <- folds[[x]]
                                   train_data <- main_impute_3[-test_id,]
                                   test_data <- main_impute_3[test_id,]
                                   
                                   #Impute the train data
                                   imp_train <- mice(train_data, method = meth_main_4,
                                                     predictorMatrix = 
                                                       predictor_matrix_main_4, 
                                                     m = m, maxit = 5)
                                   ...
                                   for (j in seq_along(imputed_test_data)) {
                                     ...
                                     los_pred_exp <- exp(los_pred)
                                     
                                     prediction_list_3[[j]] <- los_pred_exp
                                   }
                                   ...
                                   for (imp in seq_along(imputed_train_data)){
                                     
                                     data_imp <- imputed_train_data[[imp]]
                                     
                                     #numeric_cols <- names(data_imp)[
                                      # sapply(data_imp, is.numeric)
                                     #]
                                     
                                     # Continuous numeric variables with >2 unique values
                                     #cols_to_scale <- numeric_cols[
                                      # sapply(
                                       #  data_imp[numeric_cols],
                                        # function(col) length(unique(col)) > 2
                                       #)
                                     #]
                                     
                                     # Never scale the outcome
                                    # cols_to_scale <- setdiff(
                                     #  cols_to_scale,
                                      # "total_los"
                                     #)
                                     
                                     # Everything else remains unscaled
                                     #other_cols <- setdiff(
                                      # names(data_imp),
                                       #cols_to_scale
                                     #)
                                     
                                     folds_2 <- createFolds(data_imp$total_los, k = 5, returnTrain = FALSE)
                                     
                                     fold_results <- data.frame(Fold = seq_along(folds_2),
                                                                RMSE = NA_real_,
                                                                MAE = NA_real_,
                                                                R_Squared = NA_real_)
                                     ...
                                     for (k in seq_along(folds_2)){
                                       
                                       test_index <- folds_2[[k]]
                                       
                                       test_data_2 <- data_imp[test_index, ]
                                       train_data_2 <- data_imp[-test_index,]
                                       
                                       #train_scaled_cont <- scale(
                                        # train_data[, cols_to_scale, drop = FALSE]
                                       #)
                                       
                                       #test_scaled_cont <- scale(
                                        # test_data[, cols_to_scale, drop = FALSE],
                                         #center = attr(train_scaled_cont, "scaled:center"),
                                         #scale = attr(train_scaled_cont, "scaled:scale")
                                       #)
                                       
                                       #train_scaled <- train_data
                                       #train_scaled[, cols_to_scale] <- train_scaled_cont
                                       
                                       #test_scaled <- test_data
                                       #test_scaled[, cols_to_scale] <- test_scaled_cont
                                       
                                       model_glm <- glm(formula = full_model_formula_3,
                                                        data = train_data_2,
                                                        family = Gamma(link = "log"))
                                       
                                       glm_models[[imp]][[k]] <- model_glm
                                       
                                       pred_log <- predict(model_glm, newdata = test_data_2)
                                       pred <- exp(pred_log)
                                       
                                       actual <- test_data_2$total_los
                                       ...
                                       fold_results$RMSE[k] <- rmse
                                       fold_results$MAE[k] <- mae
                                       fold_results$R_Squared[k] <- r_squared
                                     }
                                     
                                     cv_results[[imp]] <- fold_results
                                   }
                                   
                                   glm_rmse <- mean(unlist(lapply(cv_results, function(x) x$RMSE)))
                                   glm_mae <- mean(unlist(lapply(cv_results, function(x) x$MAE)))
                                   glm_r_squared <- mean(unlist(lapply(cv_results, function(x) x$R_Squared)))
                                   ...
                                   for (i in seq_along(imputed_datasets)) {
                                     ...
                                     data_imp_2 <- imputed_datasets[[i]]
                                     
                                     # Create 5 folds for this imputed dataset
                                     folds_3 <- createFolds(
                                       data_imp_2$total_los,
                                       k = 5,
                                       returnTrain = FALSE
                                     )
                                     
                                     fold_results_2 <- data.frame(
                                       Fold = seq_along(folds_3),
                                       RMSE = NA_real_,
                                       MAE = NA_real_,
                                       R_Squared = NA_real_
                                     )
                                     ...
                                     for (k in seq_along(folds_3)) {
                                       
                                       # Test and training data
                                       test_index <- folds_3[[k]]
                                       
                                       test_data_3 <- data_imp_2[test_index, , drop = FALSE]
                                       train_data_3 <- data_imp_2[-test_index, , drop = FALSE]
                                       ...
                                       fold_results_2$RMSE[k] <- rmse
                                       fold_results_2$MAE[k] <- mae
                                       fold_results_2$R_Squared[k] <- r_squared
                                     }
                                     
                                     rf_cv_results[[i]] <- fold_results_2
                                   }
                                   ...
                                   for (i in seq_along(rfm_model)) {
                                     
                                     for (k in seq_along(rfm_model[[i]])) {
                                       
                                       imp <- importance(rfm_model[[i]][[k]])
                                       
                                       rf_importance[[length(rf_importance) + 1]] <- data.frame(
                                         Imputation = i,
                                         Fold = k,
                                   ...
                                   list(lm_rmse = lm_rmse,
                                        ...
                                        #   xgboost_single = single_tree,
                                        #   xgboost = xgboost_multi
                                        train_data2 = imputed_train_data,
                                        rmse_list = rmse_list_lm,
                                        glm_rmse = glm_rmse,
                                        glm_mae = glm_mae,
                                        glm_rsquared = glm_r_squared,
                                        rf_rmse = rf_rmse,
                                        rf_mae = rf_mae,
                                        rf_r_squared = rf_r_squared,
...
hh <- as.data.frame(results_3[[1]][["rf_importance"]])

plot_model(results_3[[1]]$lm_models[[1]], show.values = TRUE, value.offset = .3)

coef <- coef(results_3[[1]]$glm_models[[1]][[1]])
ci_2 <- confint(results_3[[1]]$glm_models[[1]][[1]])

df_3 <- data.frame(variable = names(coef),
estimate = exp(coef),
lower = exp(ci_2[,1]),
upper = exp(ci_2[,2]))

df_3 <- df_3[df_3$variable != "(Intercept)", ]
df_3$label <- sprintf("%.2f", df_3$estimate)

rownames(df_3) <- c("Age", "Sex: Male", "Admission Type: Emergency", "Admission Type: Observation",
                   "Admission Type: Urgent", "Ethnicity: Black", "Ethnicity: Hispanic/Latino", 
                   "Ethnicity: Other", "Ethnicity: White", "Insurance: Medicare", "Insurance: Other",
                   "Insurance: Private", "Platelets", "Glucose", "Chloride", "Potassium", "Haemoglobin",
                   "Sodium", "Ondansetron", "Calcium Gluconate")

df_3$variable <- c("Age", "Sex: Male", "Admission Type: Emergency", "Admission Type: Observation",
                  "Admission Type: Urgent", "Ethnicity: Black", "Ethnicity: Hispanic/Latino", 
                  "Ethnicity: Other", "Ethnicity: White", "Insurance: Medicare", "Insurance: Other",
                  "Insurance: Private", "Platelets", "Glucose", "Chloride", "Potassium", "Haemoglobin",
                  "Sodium", "Ondansetron",
                  "Calcium Gluconate")[match(df_3$variable, c("age", "sexM", "admission_typeEmergency",
                                                             "admission_typeObservation", "admission_typeUrgent",
                                                             "ethnicityBlack", "ethnicityHispanic/Latino",
                                                             "ethnicityOther", "ethnicityWhite", "insuranceMedicare",
                                                             "insuranceOther", "insurancePrivate", "platelets", "glucose",
                                                             "chloride", "potassium", "haemoglobin", "sodium", 
                                                             "ondansetron1", "calcium_gluconate1"))]

ggplot(df_3, aes(x = estimate, y = reorder(variable, estimate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2, alpha = 0.2) +
  #geom_vline(xintercept = 1, linetype = "dashed") +
  geom_text(aes(label = label), hjust = -0.1, size = 3, fontface = "bold") +
  xlab("Multiplicative effect on length of stay") +
  ylab("Variable") +
  theme_igray() +
  ggtitle("Gamma GLM Forest Plot") +
  xlim(0, max(df_3$upper) * 1.2) +
  coord_cartesian(xlim = c(0,2)) +
  theme(plot.title = element_text(hjust = 0.5))


coef_est <- coef(results_3[[1]]$lm_models[[1]])
ci <- confint(results_3[[1]]$lm_models[[1]])

coef_est_1_1 <- coef(results_3[[1]]$lm_models[[1]])
ci_1_1 <- confint(results_3[[1]]$lm_models[[1]])

coef_est_1_2 <- coef(results_3[[1]]$lm_models[[2]])
ci_1_2 <- confint(results_3[[1]]$lm_models[[2]])

coef_est_1_3 <- coef(results_3[[1]]$lm_models[[3]])
ci_1_3 <- confint(results_3[[1]]$lm_models[[3]])

coef_est_1_4 <- coef(results_3[[1]]$lm_models[[4]])
ci_1_4 <- confint(results_3[[1]]$lm_models[[4]])

coef_est_1_5 <- coef(results_3[[1]]$lm_models[[5]])
ci_1_5 <- confint(results_3[[1]]$lm_models[[5]])

coef_est_2_1 <- coef(results_3[[2]]$lm_models[[1]])
ci_2_1 <- confint(results_3[[2]]$lm_models[[1]])

coef_est_2_2 <- coef(results_3[[2]]$lm_models[[2]])
ci_2_2 <- confint(results_3[[2]]$lm_models[[2]])

coef_est_2_3 <- coef(results_3[[2]]$lm_models[[3]])
ci_2_3 <- confint(results_3[[2]]$lm_models[[3]])

coef_est_2_4 <- coef(results_3[[2]]$lm_models[[4]])
ci_2_4 <- confint(results_3[[2]]$lm_models[[4]])

coef_est_2_5 <- coef(results_3[[2]]$lm_models[[5]])
ci_2_5 <- confint(results_3[[2]]$lm_models[[5]])

coef_est_3_1 <- coef(results_3[[3]]$lm_models[[1]])
ci_3_1 <- confint(results_3[[3]]$lm_models[[1]])

coef_est_3_2 <- coef(results_3[[3]]$lm_models[[2]])
ci_3_2 <- confint(results_3[[3]]$lm_models[[2]])

coef_est_3_3 <- coef(results_3[[3]]$lm_models[[3]])
ci_3_3 <- confint(results_3[[3]]$lm_models[[3]])

coef_est_3_4 <- coef(results_3[[3]]$lm_models[[4]])
ci_3_4 <- confint(results_3[[3]]$lm_models[[4]])

coef_est_3_5 <- coef(results_3[[3]]$lm_models[[5]])
ci_3_5 <- confint(results_3[[3]]$lm_models[[5]])

coef_est_4_1 <- coef(results_3[[4]]$lm_models[[1]])
ci_4_1 <- confint(results_3[[4]]$lm_models[[1]])

coef_est_4_2 <- coef(results_3[[4]]$lm_models[[2]])
ci_4_2 <- confint(results_3[[4]]$lm_models[[2]])

coef_est_4_3 <- coef(results_3[[4]]$lm_models[[3]])
ci_4_3 <- confint(results_3[[4]]$lm_models[[3]])

coef_est_4_4 <- coef(results_3[[4]]$lm_models[[4]])
ci_4_4 <- confint(results_3[[4]]$lm_models[[4]])

coef_est_4_5 <- coef(results_3[[4]]$lm_models[[5]])
ci_4_5 <- confint(results_3[[4]]$lm_models[[5]])

coef_est_5_1 <- coef(results_3[[5]]$lm_models[[1]])
ci_5_1 <- confint(results_3[[5]]$lm_models[[1]])

coef_est_5_2 <- coef(results_3[[5]]$lm_models[[2]])
ci_5_2 <- confint(results_3[[5]]$lm_models[[2]])

coef_est_5_3 <- coef(results_3[[5]]$lm_models[[3]])
ci_5_3 <- confint(results_3[[5]]$lm_models[[3]])

coef_est_5_4 <- coef(results_3[[5]]$lm_models[[4]])
ci_5_4 <- confint(results_3[[5]]$lm_models[[4]])

coef_est_5_5 <- coef(results_3[[5]]$lm_models[[5]])
ci_5_5 <- confint(results_3[[5]]$lm_models[[5]])

df2 <- data.frame(
  variable = names(coef_est_1_1),
  estimate_1_1 = exp(coef_est_1_1),
  lower_1_1 = exp(ci_1_1[,1]),
  upper_1_1 = exp(ci_1_1[,2]),
  estimate_1_2 = exp(coef_est_1_2),
  lower_1_2 = exp(ci_1_2[,1]),
  upper_1_2 = exp(ci_1_2[,2]),
  estimate_1_3 = exp(coef_est_1_3),
  lower_1_3 = exp(ci_1_3[,1]),
  upper_1_3 = exp(ci_1_3[,2]),
  estimate_1_4 = exp(coef_est_1_4),
  lower_1_4 = exp(ci_1_4[,1]),
  upper_1_4 = exp(ci_1_4[,2]),
  estimate_1_5 = exp(coef_est_1_5),
  lower_1_5 = exp(ci_1_5[,1]),
  upper_1_5 = exp(ci_1_5[,2]),
  estimate_2_1 = exp(coef_est_2_1),
  lower_2_1 = exp(ci_2_1[,1]),
  upper_2_1 = exp(ci_2_1[,2]),
  estimate_2_2 = exp(coef_est_2_2),
  lower_2_2 = exp(ci_2_2[,1]),
  upper_2_2 = exp(ci_2_2[,2]),
  estimate_2_3 = exp(coef_est_2_3),
  lower_2_3 = exp(ci_2_3[,1]),
  upper_2_3 = exp(ci_2_3[,2]),
  estimate_2_4 = exp(coef_est_2_4),
  lower_2_4 = exp(ci_2_4[,1]),
  upper_2_4 = exp(ci_2_4[,2]),
  estimate_2_5 = exp(coef_est_2_5),
  lower_2_5 = exp(ci_2_5[,1]),
  upper_2_5 = exp(ci_2_5[,2]),
  estimate_3_1 = exp(coef_est_3_1),
  lower_3_1 = exp(ci_3_1[,1]),
  upper_3_1 = exp(ci_3_1[,2]),
  estimate_3_2 = exp(coef_est_3_2),
  lower_3_2 = exp(ci_3_2[,1]),
  upper_3_2 = exp(ci_3_2[,2]),
  estimate_3_3 = exp(coef_est_3_3),
  lower_3_3 = exp(ci_3_3[,1]),
  upper_3_3 = exp(ci_3_3[,2]),
  estimate_3_4 = exp(coef_est_3_4),
  lower_3_4 = exp(ci_3_4[,1]),
  upper_3_4 = exp(ci_3_4[,2]),
  estimate_3_5 = exp(coef_est_3_5),
  lower_3_5 = exp(ci_3_5[,1]),
  upper_3_5 = exp(ci_3_5[,2]),
  estimate_4_1 = exp(coef_est_4_1),
  lower_4_1 = exp(ci_4_1[,1]),
  upper_4_1 = exp(ci_4_1[,2]),
  estimate_4_2 = exp(coef_est_4_2),
  lower_4_2 = exp(ci_4_2[,1]),
  upper_4_2 = exp(ci_4_2[,2]),
  estimate_4_3= exp(coef_est_4_3),
  lower_4_3 = exp(ci_4_3[,1]),
  upper_4_3 = exp(ci_4_3[,2]),
  estimate_4_4 = exp(coef_est_4_4),
  lower_4_4 = exp(ci_4_4[,1]),
  upper_4_4 = exp(ci_4_4[,2]),
  estimate_4_5 = exp(coef_est_4_5),
  lower_4_5 = exp(ci_4_5[,1]),
  upper_4_5 = exp(ci_4_5[,2]),
  estimate_5_1 = exp(coef_est_5_1),
  lower_5_1 = exp(ci_5_1[,1]),
  upper_5_1 = exp(ci_5_1[,2]),
  estimate_5_2 = exp(coef_est_5_2),
  lower_5_2 = exp(ci_5_2[,1]),
  upper_5_2 = exp(ci_5_2[,2]),
  estimate_5_3 = exp(coef_est_5_3),
  lower_5_3 = exp(ci_5_3[,1]),
  upper_5_3 = exp(ci_5_3[,2]),
  estimate_5_4 = exp(coef_est_5_4),
  lower_5_4 = exp(ci_5_4[,1]),
  upper_5_4 = exp(ci_5_4[,2]),
  estimate_5_5 = exp(coef_est_5_5),
  lower_5_5 = exp(ci_5_5[,1]),
  upper_5_5 = exp(ci_5_5[,2])
)

df2 <- df2 %>%
  mutate(estimate = rowMeans(across(c(estimate_1_1, estimate_1_2, estimate_1_3, estimate_1_4,
                                    estimate_1_5, estimate_2_1, estimate_2_2, estimate_2_3,
                                    estimate_2_4, estimate_2_5, estimate_3_1, estimate_3_2,
                                    estimate_3_3, estimate_3_4, estimate_3_5, estimate_4_1,
                                    estimate_4_2, estimate_4_3, estimate_4_4, estimate_4_5,
                                    estimate_5_1, estimate_5_2, estimate_5_3, estimate_5_4,
                                    estimate_5_5)))) %>%
  mutate(lower = rowMeans(across(c(lower_1_1, lower_1_2, lower_1_3, lower_1_4,
                                      lower_1_5, lower_2_1, lower_2_2, lower_2_3,
                                      lower_2_4, lower_2_5, lower_3_1, lower_3_2,
                                      lower_3_3, lower_3_4, lower_3_5, lower_4_1,
                                      lower_4_2, lower_4_3, lower_4_4, lower_4_5,
                                      lower_5_1, lower_5_2, lower_5_3, lower_5_4,
                                      lower_5_5)))) %>%
  mutate(upper = rowMeans(across(c(upper_1_1, upper_1_2, upper_1_3, upper_1_4,
                                      upper_1_5, upper_2_1, upper_2_2, upper_2_3,
                                      upper_2_4, upper_2_5, upper_3_1, upper_3_2,
                                      upper_3_3, upper_3_4, upper_3_5, upper_4_1,
                                      upper_4_2, upper_4_3, upper_4_4, upper_4_5,
                                      upper_5_1, upper_5_2, upper_5_3, upper_5_4,
                                      upper_5_5)))) %>%
  select(-c(estimate_1_1, estimate_1_2, estimate_1_3, estimate_1_4,
            estimate_1_5, estimate_2_1, estimate_2_2, estimate_2_3,
            estimate_2_4, estimate_2_5, estimate_3_1, estimate_3_2,
            estimate_3_3, estimate_3_4, estimate_3_5, estimate_4_1,
            estimate_4_2, estimate_4_3, estimate_4_4, estimate_4_5,
            estimate_5_1, estimate_5_2, estimate_5_3, estimate_5_4,
            estimate_5_5,lower_1_1, lower_1_2, lower_1_3, lower_1_4,
            lower_1_5, lower_2_1, lower_2_2, lower_2_3,
            lower_2_4, lower_2_5, lower_3_1, lower_3_2,
            lower_3_3, lower_3_4, lower_3_5, lower_4_1,
            lower_4_2, lower_4_3, lower_4_4, lower_4_5,
            lower_5_1, lower_5_2, lower_5_3, lower_5_4,
            lower_5_5,upper_1_1, upper_1_2, upper_1_3, upper_1_4,
            upper_1_5, upper_2_1, upper_2_2, upper_2_3,
            upper_2_4, upper_2_5, upper_3_1, upper_3_2,
            upper_3_3, upper_3_4, upper_3_5, upper_4_1,
            upper_4_2, upper_4_3, upper_4_4, upper_4_5,
...
df$label <- sprintf("%.2f", df$estimate)
df2$label <- sprintf("%.2f", df2$estimate)

ggplot(df, aes(x = estimate, y = reorder(variable, estimate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2) +
  geom_vline(xintercept = 1, linetype = "dashed") +
  geom_text(aes(label = label), hjust = -0.1, size = 3) +
  xlab("Multiplicative effect on LOS (exp(beta))") +
  ylab("") +
  theme_minimal() +
  xlim(0, max(df$upper) * 1.2)

rownames(df2) <- c("Age", "Sex: Male", "Admission Type: Emergency", "Admission Type: Observation",
                   "Admission Type: Urgent", "Ethnicity: Black", "Ethnicity: Hispanic/Latino", 
                   "Ethnicity: Other", "Ethnicity: White", "Insurance: Medicare", "Insurance: Other",
                   "Insurance: Private", "Platelets", "Glucose", "Chloride", "Potassium", "Haemoglobin",
                   "Sodium", "Ondansetron", "Calcium Gluconate")

df2$variable <- c("Age", "Sex: Male", "Admission Type: Emergency", "Admission Type: Observation",
                  "Admission Type: Urgent", "Ethnicity: Black", "Ethnicity: Hispanic/Latino", 
                  "Ethnicity: Other", "Ethnicity: White", "Insurance: Medicare", "Insurance: Other",
                  "Insurance: Private", "Platelets", "Glucose", "Chloride", "Potassium", "Haemoglobin",
                  "Sodium", "Ondansetron",
                  "Calcium Gluconate")[match(df2$variable, c("age", "sexM", "admission_typeEmergency",
                                                             "admission_typeObservation", "admission_typeUrgent",
                                                             "ethnicityBlack", "ethnicityHispanic/Latino",
                                                             "ethnicityOther", "ethnicityWhite", "insuranceMedicare",
                                                             "insuranceOther", "insurancePrivate", "platelets", "glucose",
...
ggplot(df2, aes(x = estimate, y = reorder(variable, estimate))) +
  geom_point() +
  geom_errorbarh(aes(xmin = lower, xmax = upper), height = 0.2, alpha = 0.2) +
  #geom_vline(xintercept = 1, linetype = "dashed") +
  geom_text(aes(label = label), hjust = -0.1, size = 3, fontface = "bold") +
...
for(i in seq_along(results_3)) {
  ...
  # Extract the variable importance matrix/vector
  imp_3 <- results_3[[i]]$variable_importance  # should be a matrix or data frame
  
  # Compute row means if it's multi-column, otherwise keep as is
  if(is.null(var_imp_3) == TRUE){
    var_imp_3 <- rowMeans(imp_3)
  } else {
    var_imp_3 <- cbind(var_imp_3, imp_3) %>%
      rowMeans()
  }
}
  ...
  for(i in seq_along(results_3)) {
    ...
    # Extract the variable importance matrix/vector
    imp_4 <- as.data.frame(results_3[[i]][["rf_importance"]])  # should be a matrix or data frame
    
    # Compute row means if it's multi-column, otherwise keep as is
    if(is.null(var_imp_4) == TRUE){
      var_imp_4 <- imp_4
    } else {
      var_imp_4 <- cbind(var_imp_4, imp_4)
    }
...
names(var_imp_4) <- make.unique(names(var_imp_4))  

var_imp_4 <- var_imp_4 %>%
  select(-Variable.1, -Variable.2, -Variable.3, -Variable.4, -SD_Importance, -SD_Importance.1, 
         -SD_Importance.2, -SD_Importance.3, -SD_Importance.4) %>%
  mutate(Importance = rowMeans(across(c(Mean_Importance, Mean_Importance.1,
                                        Mean_Importance.2, Mean_Importance.3,
                                        Mean_Importance.4)))) %>%
  select(-Mean_Importance, -Mean_Importance.1, -Mean_Importance.2,
         -Mean_Importance.3, -Mean_Importance.4) %>%
...
var_imp_3 <- as.data.frame(var_imp_3) %>%
  rename(importance = var_imp_3)

rownames(var_imp_3) <- c("Age", "Sex", "Admission Type", "Ethnicity", "Insurance", 
                         "Platelets", "Glucose", "Chloride", "Potassium", "Haemoglobin",
                         "Sodium", "Ondansetron", "Calcium Gluconate") 
...
var_imp_4$Variable<- var_imp_4$Variable %>% as.factor()
var_imp_4$Variable <- fct_inorder(var_imp_4$Variable)
...
imp_bar_3 + coord_flip() + theme_igray()

imp_bar_4 <- ggplot(data = var_imp_4) + 
  geom_bar(
    stat = "identity",#it leaves the data without count and bin
    mapping = aes(x = Variable, y=Importance, fill = Variable), 
    show.legend = FALSE,
    width = 1
  ) + 
  labs(x = "Variable", y = "Variable importance") +
...
imp_bar_4 + coord_polar() + theme_minimal()
imp_bar_4 + coord_flip() + theme_igray()
